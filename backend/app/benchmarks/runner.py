...
def run_python_sample(
    ...
    limit: int = 10,
    bank: PuzzleBank | None = None,
    solver: PythonSudokuSolver | None = None,
) -> list[SimulationResult]:
    return run_solver_sample(
        solver=solver or PythonSudokuSolver(),
...
def run_solver_sample(
    solver: PythonSudokuSolver | PrologSudokuSolver,
    difficulty: str = "Easy",
    limit: int = 10,
    bank: PuzzleBank | None = None,
) -> list[SimulationResult]:
    ...
    results: list[SimulationResult] = []
    for record in puzzle_bank.by_difficulty(difficulty)[:limit]:
        result = solver.solve(record.puzzle, SolverOptions(explain=False))
        results.append(
            SimulationResult(
                puzzle_id=record.id,
                difficulty=record.difficulty,
